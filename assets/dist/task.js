...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderTasks = (tasks) => {
        ...
        <td>
          <button class="btn btn-sm btn-primary" onclick="editTask(${task.id}, '${sanitize(task.title)}', '${sanitize(task.description)}')">Edit</button>
    ...
    // Delete a task
    window.deleteTask = (id) => __awaiter(void 0, void 0, void 0, function* () {
        if (!confirm('Are you sure you want to delete this task?')) {
            return;
        }
        try {
            const response = yield fetch(`${apiBaseUrl}/${id}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`,
                },
